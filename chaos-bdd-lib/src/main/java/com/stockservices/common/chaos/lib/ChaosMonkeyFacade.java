...
public class ChaosMonkeyFacade {
    ...
    public void enableChaos() {
        ResponseEntity<String> response = unmanagedRestTemplate.postForEntity(getBaseUrl() + CHAOS_MONKEY_ENABLE_URL, null, String.class);
    ...
    public void disableChaos() {
        ResponseEntity<String> response = unmanagedRestTemplate.postForEntity(getBaseUrl() + CHAOS_MONKEY_DISABLE_URL, null, String.class);
        if (!response.getStatusCode().is2xxSuccessful()) {
    ...
    public void updateWatchers(boolean controller, boolean restController, boolean service, boolean repository, boolean component, boolean restTemplate, boolean webClient) {
        Map<String, Object> watchers = new HashMap<>();
        watchers.put("controller", controller);
        watchers.put("restController", restController);
        watchers.put("service", service);
        watchers.put("repository", repository);
        watchers.put("component", component);
        watchers.put("restTemplate", restTemplate);
        watchers.put("webClient", webClient);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<Map<String, Object>> request = new HttpEntity<>(watchers, headers);
        ResponseEntity<String> response = unmanagedRestTemplate.postForEntity(getBaseUrl() + CHAOS_MONKEY_WATCHERS_URL, request, String.class);
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new RuntimeException("Failed to update watchers: " + response.getStatusCode());
        }
    }
    ...
    private void updateAssaults(Map<String, Object> assaults) {
        ...
        ResponseEntity<String> response = unmanagedRestTemplate.postForEntity(getBaseUrl() + CHAOS_MONKEY_ASSAULTS_URL, request, String.class);
        if (!response.getStatusCode().is2xxSuccessful()) {
